$(function () {
  ...
  function placesSearch () {
    ...
    $.ajax({
      ...
      success: function (places, reqStatus) {
	    ...
	    <div class="user">
          ...
          $('section.places article').prepend(aPlace);
    ...
    toggleSpans = $('div.toggle');
  }

  function fetchReviews(event) {
      // get the UL element to populate
      let reviews = '';
      $.each($('ul.hide'), function (i, item) {
         /* if ($(item).data('place') === $('toggle').data('id')) {
           review = 
         }*/
         console.log(item)
      });
  ...
  // $('div.toggle').on('click', fetchReviews);
  console.log(toggleSpans);
});
